...
(async () => {
  ...
  assert(whdr.length >= 4 && whdr.every(x => /Drop for him/.test(x.textContent) && /Verdict/.test(x.textContent)), "every waiver table has 'Drop for him' and 'Verdict' columns");
  assert([...d.querySelectorAll("#tab-waivers .gw .row.grid")].every(r => r.children.length === whdr[0].children.length && r.children[13].textContent.trim().length > 0 && r.children[14].textContent.trim().length > 0), "every waiver row fills the drop and verdict cells");
